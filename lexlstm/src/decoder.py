...
class Decoder(nn.Module):
    def __init__(
        ...
        if self.concat_feed:
            self.combine = nn.Linear(
                n_embed + n_hidden * (2 + len(attention)), n_hidden
            )
            self.rnn = self.rnntype(
                n_embed + n_hidden, n_hidden, n_layers, dropout=dropout
            )
        else:
            self.combine = nn.Linear(
                n_embed + n_hidden * (1 + len(attention)), n_hidden
            )
    ...
    def step(
        ...
        if FLAGS.highdrop:
            if FLAGS.highdroptest:
                comb_features = F.dropout(
                    comb_features, p=self.high_drop.p, training=True
                )
            else:
        ...
        if self.copy:
            ...
            if len(dists) > 1:
                ...
                weighted_dists = [
                    dists[i] * seq_probs[:, :, i] for i in range(len(dists))
                ]
            else:
            ...
            copy_probs = [
                (weighted_dists[i].unsqueeze(2) * projs[i]).sum(0)
            ...
            if type(self.copy_translation) == SoftAlign:
                ...
                for i in range(tokens.shape[0]):
                    if self.copy_translation is None:
                        ...
                        proj[
                            i,
                            range(tokens.shape[1]),
                            self.copy_translation[tokens[i, :]],
                        ] = 1
    ...
    def forward(
        ...
        if att_features is None:
            ...
            assert len(self.attention) == len(att_features) + (
                1 if self.self_attention else 0
            )
        ...
        for t in range(max_len):
            ...
            if (
                t == self.MAXLEN - 1
            ):  # FIXME: when sampling there is one more extra timestep
                new_pred = torch.zeros_like(pred)
    ...
    def sample(
        ...
        if self.rnntype == nn.LSTM:
            if rnn_state is None:
                rnnstate = tuple(
                    torch.zeros(self.n_layers, n_batch, self.n_hidden).to(device)
                    for _ in range(2)
                )
            else:
        ...
        if calc_score:
            ...
            tok_out = [
                [self.vocab.sos()] + trim(row.tolist(), self.vocab.eos())
                for i, row in enumerate(tok_arr)
            ]
            score_out = None
    ...
    def forward_onehot(
        ...
        if att_features is None:
            ...
            att_masks = tuple((toks == self.vocab.pad()).float() for toks in att_tokens)
            att_token_proj = tuple(self._make_projection(toks) for toks in att_tokens)
    ...
    def logprob(self, ref_tokens, rnn_state=None, att_features=None, att_tokens=None):
        ...
        if len(ref_tokens.shape) == 3:
            ...
            if FLAGS.debug:
                ...
                pdb.set_trace()
            out_tgt = ref_tokens[1:, :]
            logprob = -self.nll(logp.permute(1, 2, 0), out_tgt.transpose(0, 1)).sum(
                dim=-1
            )
        return logprob
        ...
        for t in range(max_len):
            ...
            decoder_state = DecoderState(
                ...
                [
                    torch.stack(
                        [
                            s.hiddens[tt] if tt < len(s.hiddens) else s.hiddens[-1]
                            for s in beam
                        ],
                        dim=1,
                    )
            ...
            pred, feed, rnn_state, hidden, *_ = self.step(
                decoder_state,
                tuple(
                    f.expand(f.shape[0], len(beam), f.shape[2]) for f in att_features
                ),
