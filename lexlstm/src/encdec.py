...
class EncDec(nn.Module):
    def __init__(
        ...
        self.nll = nn.CrossEntropyLoss(
            ignore_index=vocab_y.pad(), reduction="sum"
        )  # TODO: why mean better?
        ...
        if embedding_file is not None:
            input_vocab = list(map(str.lower, vocab_x.contents.keys()))
            output_vocab = list(map(str.lower, vocab_y.contents.keys()))
            allwords = set(input_vocab + output_vocab)
            utils.load_from_saved_embeddings(
                embedding_file,
                vocab_x,
    ...
    def pass_hiddens(self, rnnstate):
        if self.rnntype == nn.LSTM:
            ...
            state = rnnstate.view(self.n_layers, -1, rnnstate.shape[1], self.dim).sum(
                dim=1
            )
    ...
    def logprob_interleaved(self, inp, out, lens=None):
        hid, state = self.encoder(inp, lens=lens)
        sbatch = [
            s.repeat_interleave(out.shape[1], dim=1) for s in self.pass_hiddens(state)
        ]
